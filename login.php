<?php
include "./include/header.php"
?>

<div class="container-fluid mt-3">
  <div class="row px-3 py-4">
    <div class="col-12 col-md-12 mt-4 d-flex justify-content-center flex-column align-items-center scale-up">
      <p class="small-text">Connect</p>
      <h1>Login to Profiles</h1>
      <p class="mt-4 dark-text">
        Welcome to Manjithaaza! Login to your smart card or company profile.
      </p>
      <!-- <div class="d-flex flex-column flex-sm-row">
            <button
              type="button"
              class="btn btn-light px-4 py-2 me-2 mb-2 mb-sm-0"
            >
              Login
            </button>
            <button type="button" class="btn btn-outline-light px-4 py-2">
              Sign Up
            </button>
          </div> -->
      <div class="mt-5">
        <!-- <h2>Access Your Profile Here</h2> -->
        <p>Enter your credentials to access your profile.</p>
        <div class="mb-3">
          <input
            type="email"
            class="form-control"
            id="exampleFormControlInput1"
            placeholder="Email Address"
            style="max-width: 100%; width: 400px" />

        </div>
        <div class="mb-3">
          <input
            type="password"
            class="form-control"
            id="exampleFormControlInput1"
            placeholder="Password"
            style="max-width: 100%; width: 400px" />

        </div>


        <button type="button" class="btn learn-more-btn">
          Log In
        </button>

      </div>
    </div>
    <div
      class="col-12 col-md-6 d-flex justify-content-center align-items-center">

      <!-- Add content here for the second column -->
    </div>
  </div>
</div>



<div class="container mt-5">
  <div class="row">
    <div class="text-center">
      <h5>Still have questions?</h5>
      <p>Contact our support team for further assistance.</p>
      <a href="./contactus.php" style="color:white">
        <button
          type="button"
          class="btn contact-us-btn px-4 py-2 flex-grow-0">
          Contact
        </button>
      </a>
    </div>
  </div>
</div>

<!-- <div class="container-fluid mt-5">
      <div class="row" style="padding: 30px">
        <div>
          <small>Support</small>
          <h4>Contact Support</h4>
          <small
            >If you need assistance with login issues, please contact our
            support team.</small
          >
        </div>
      </div>
    </div> -->

<?php

include "./include/footer.php"
?>